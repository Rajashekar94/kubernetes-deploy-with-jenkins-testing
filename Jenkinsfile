...
stage('Build image') {
   sshagent(['kubernetes-cluster']) {
    sh "scp -o StrictHostKeyChecking=no frontend-deployment.yml frontend-svc.yml ubuntu@34.68.5.242:/home/ubuntu/guestbook"
        script{
	      try {
		   sh "ssh  ubuntu@34.68.5.242 kubectl apply -f frontend-deployment.yml frontend-svc.yml"
		   } catch(error){
           sh "ssh  ubuntu@34.68.5.242 kubectl create -f frontend-deployment.yml frontend-svc.yml"  
	       }
		   }
}
}
